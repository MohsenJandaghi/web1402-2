...
function readRecordController() {
    fs.readFile('Records.txt', {encoding: 'utf8'}, function(err, fileData){
        if(err){
            console.log('ERR: ', err);
        }
        else {
            ...
            for (let x in fileData){
                console.log(x.Records.id)
                if(x.Records.id == process.argv[3]){
                    obejectR=x.Records;
                    console.log(obejectR);
                }
                else {
    ...
    createRecord: function(){
        fs.readFile('Records.txt', {encoding: 'utf8'}, function(err, fileData){
            if(err){
                ...
                fileData = JSON.parse(fileData);
                let data = {
                    id: fileData.Records.length+100,
                    name: process.argv[3],
                ...
                fs.writeFile('Records.txt', fileData, fsCallback);
            }
        });
    },
