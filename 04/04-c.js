...
let commands = {
    ...
    createRecord: function(){
        ...
        fs.readFile('dsf.txt', {encoding: 'utf8'}, function(err, fileData){
            if(err){
                console.log('ERR: ', err);
            }
            else {
                fileData = JSON.parse(fileData);
                fileData.myRecords.push(data);
                fileData = JSON.stringify(fileData);
                fs.writeFile('dsf.txt', fileData, fsCallback);
            }
        });
    },
    readRecord: function(){
        fs.readFile('dsf.txt', {encoding: 'utf8'}, function(err, fileData){
            if(err){
                ...
                fileData = JSON.parse(fileData);
                obejectR=fileData.myRecords[process.argv[3]];
